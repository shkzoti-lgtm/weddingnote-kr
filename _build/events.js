...
(function(){
  ...
  function alwaysLabel(start,end,status){
    ...
    // 시작일이 비었는데 종료일·상태에 '매주 토요일/일요일' 같은 반복 표현만 남은 경우
    if(!st){
      var RECUR=/매주|매월|주말|평일|연중|상시|수시|상설/, DATE=/\d{4}\D+\d{1,2}\D+\d{1,2}/;
      for(var i=0;i<2;i++){ var v=i?sc:en; if(v&&RECUR.test(v)&&!DATE.test(v)) return v; }
    }
    return null;
  }
